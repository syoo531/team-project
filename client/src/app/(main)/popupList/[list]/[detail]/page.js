...
export default function PopUp(props) {
  ...
  const fetchData = async () => {
    try {
      ...
      const reviewDataResponse = await instance.get(
        `review/byPopupstore/${storeId}`,
      );
    ...
    <div className="PopUp">
      ...
      <div className="popReview">
        <h4>
          ...
          <Link href={`/popupList/all/${props.params.detail}/reviews`}>
            전체보기
          </Link>
        </h4>
        <div className="popReviewList">
          {reviewData.map((review, index) => (
            <div className="popReviewListItem" key={index}>
              ...
              <button
                type="button"
                className="imgToggleBtn"
                onClick={() => toggleReviewImgWrap(index)}
              >
                {getButtonText(index)}
              </button>
              <div className="img">
                <div
                  className={`img_wrap ${
                    reviewImgWrapOpen[index] ? "open" : "closed"
                  }`}
                >
      ...
      <div className="popWarning">
        ...
        <ul>
          <li>
            * 별도의 주차 장소가 없으니 대중교통과 주변 공용 주차장 이용 부탁
            드립니다.
          </li>
        </ul>
      </div>
