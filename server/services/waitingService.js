...
class WaitingService {
    ...
    async getWaitingStatus(email) {
        ...
        const waiting = await Waiting.find({ user, is_enter: false }).select("popup_store");

        let result = [];
        if (waiting.length !== 0) {
            for (let v of waiting) {
                const popup = v.popup_store;
                const popup_info = await PopupStore.findOne({ _id: popup }).select("name");
    ...
    async validateAdmin(email, popupStoreId) {
        ...
        if (isAdminUser && isAdminUser.admin_corp === popupStoreId && isAdminUser.admin_corp !== undefined) {
            return true;
        } else {
            return false;
        }
