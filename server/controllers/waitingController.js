...
const { NotFoundError, BadRequestError, InternalServerError, ConflictError } = require("../config/customError");
...
const createWaiting = async (req, res, next) => {
    try {
        ...
        const validateWaitingService = await waitingService.validateWaiting(email, popup);
        ...
        return res.status(200).json({ message: "현장대기 예약 완료입니다.", data: beforeMe });
    } catch (error) {
        next(error);
    }
};
...
const getPopupStoreId = async (req, res, next) => {
    try {
        ...
        if (!popupStore) {
            return res.status(404).json({ error: "해당 팝업스토어 Id가 존재하지 않습니다." });
        }
...
const updateWaitingPeople = async (req, res, next) => {
    try {
        ...
        const waitingService = new WaitingService();
        const updateWaitingPeople = await waitingService.updateWaitingPeople(userId, popupStoreId, people);
...
const deleteWaitingPeople = async (req, res, next) => {
    try {
        ...
        const waitingService = new WaitingService();
        const deleteWaitingPeople = await waitingService.deleteWaitingPeople(popupStoreId);
...
const enterWaitingList = async (req, res, next) => {
    try {
        ...
        const waitingService = new WaitingService();
        const completedWaiting = await waitingService.enterWaitingList(popupStoreId, userId);
