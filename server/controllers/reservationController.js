...
const getReservationByCorpAdmin = async (req, res) => {
    try {
        ...
        const reservationService = new ReservationService();
        const reservationList = await reservationService.getReservationByCorpAdmin(popupStoreId);
        if (!reservationList) {
            throw new NotFoundError("조회되는 예약이 없습니다!");
        }
...
const enterReservation = async (req, res, next) => {
    try {
        ...
        const reservationService = new ReservationService();
        const completedReservation = await reservationService.enterReservation(popupStoreId, userId);
...
const deleteReservation = async (req, res, next) => {
    try {
        const reservationService = new ReservationService();
        const deletedReservation = await reservationService.deleteReservation(req.params.id, req.body);
